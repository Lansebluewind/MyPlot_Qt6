...
void CustomChartView::exportCsvRequested()
{
	// 获取文件保存路径
	QString fileName = QFileDialog::getSaveFileName(this, "导出为CSV", "", "CSV Files (*.csv)");
	if (fileName.isEmpty())
		return;
	...
	// 假设所有曲线点数和x坐标完全一致
	int pointCount = lines.first()->points().size();
	for (int i = 0; i < pointCount; ++i)
	{
		double x = lines.first()->points().at(i).x();
		out << x;
		for (auto* line : lines)
		{
